...
public class ObjMtl_Separator extends PApplet {
...
class Rectangle
{
  ...
  public void draw(int border, int fill)
  {
    stroke(border);
    fill(fill);
...
class RectangleText
{
  ...
  RectangleText(float x, float y, float w, float h, int borderC, int bgC, String default_text, int textC)
  {
  ...
  public void draw()
  {
    ...
    if (adjustLongText)
    {
      ...
      if (textW > rectangle.size.x)
      {
        ...
        float offsetWMod = mouseX / PApplet.parseFloat(width);
        offsetWMod = constrain(offsetWMod * 2.0f - 0.5f, 0.0f, 1.0f);
        offsetW *= offsetWMod;
      }
    }
...
class Button
{
  ...
  Button(float x, float y, float w, float h, int borderC, int c1, int c2, int c3)
  {
    rectangle = new Rectangle(x, y, w, h);
    borderColor = borderC;
    restingColor = c1;
...
//
RectangleText selectMTL_info = new RectangleText(padding, padding, 640 - padding * 2.0f, 24, color(150), color(20), "MTL File", color(255));

Button selectMTL_btn = new Button(padding, padding + yOffset, 24, 24, color(150), color(255), color(175), color(105));
RectangleText selectMTL_text = new RectangleText(24 + padding * 2.0f, padding + yOffset, 100, 24, color(150), color(20), "N/A", color(255));

//
RectangleText selectObj_info = new RectangleText(padding, padding + yOffset * 3.0f, 640 - padding * 2.0f, 24, color(150), color(20), "Obj File", color(255));

Button selectObj_btn = new Button(padding, padding + yOffset * 4.0f, 24, 24, color(150), color(255), color(175), color(105));
RectangleText selectObj_text = new RectangleText(24 + padding * 2.0f, padding + yOffset * 4.0f, 100, 24, color(150), color(20), "N/A", color(255));

//
RectangleText selectOutput_info = new RectangleText(padding, padding + yOffset * 6.0f, 640 - padding * 2.0f, 24, color(150), color(20), "Output Directory", color(255));

Button selectOutput_btn = new Button(padding, padding + yOffset * 7.0f, 24, 24, color(150), color(255), color(175), color(105));
RectangleText selectOutput_text = new RectangleText(24 + padding * 2.0f, padding + yOffset * 7.0f, 100, 24, color(150), color(20), "", color(255));

//
Button render_btn = new Button(padding, padding + yOffset * 9.0f, 24, 24, color(150), color(255), color(175), color(105));
RectangleText render_text = new RectangleText(24 + padding * 2.0f, padding + yOffset * 9.0f, 100, 24, color(150), color(20), "Compile Objs", color(255));
...
public void draw()
{
  ...
  colorMode(RGB);
  //
  selectMTL_info.rectangle.size.x = width - padding * 2.0f;
  selectMTL_info.draw();
  ...
  selectMTL_text.rectangle.size.x = width - padding * 3.0f - 24;
  selectMTL_text.draw();
  //
  selectObj_info.rectangle.size.x = width - padding * 2.0f;
  selectObj_info.draw();

  selectObj_btn.update();
  selectObj_btn.draw();

  selectObj_text.rectangle.size.x = width - padding * 3.0f - 24;
  selectObj_text.draw();
  //
  selectOutput_info.rectangle.size.x = width - padding * 2.0f;
  selectOutput_info.draw();

  selectOutput_btn.update();
  selectOutput_btn.draw();

  selectOutput_text.rectangle.size.x = width - padding * 3.0f - 24;
...
public boolean Overwrite_Accept(String fileName)
{
...
public void compileFiles()
{
  ...
  //for (Map.Entry me : MaterialDiffuseColors.entrySet())
  {
    ...
    int px = PApplet.parseInt(p.x * 255.0f);
    int py = PApplet.parseInt(p.y * 255.0f);
    int pz = PApplet.parseInt(p.z * 255.0f);
    //
...
public void fileSelected(File selection) {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "ObjMtl_Separator" };
    if (passedArgs != null) {
